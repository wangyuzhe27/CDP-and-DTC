...
__all__ = ['ResNet56']  # 当其它文件以“from 模块名 import *”的形式导入该模块时,该文件中只能使用 __all__ 列表中指定的成员
...
class ResNet56(nn.Module):
    ...
    def update_skeleton(self, threshold):
        for key, m in self.named_modules():
            if isinstance(m, FilterStripe) and 'downsample' not in key:
                m.update_skeleton_skeleton(threshold)
                out_mask = m.update_skeleton()
    ...
    def prune(self, threshold):
        for key, m in self.named_modules():
            if key.startswith('conv'):
                ...
                if (m.conv1.FilterSkeleton.data.abs() > threshold).sum() == 0 or (
                        m.conv2.FilterSkeleton.data.abs() > threshold).sum() == 0:
